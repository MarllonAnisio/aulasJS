...
// testando
const cpf1 = "468.862.130-24"; // Exemplo (fictício)
const cpf2 = "234-72047336"; // Tamanho errado/inválido

console.log(`CPF 1 Válido? ${ValidaCPF.validarCPF(cpf1)}`);
console.log(`CPF 2 Válido? ${ValidaCPF.validarCPF(cpf2)}`);

// 1. Validando um CPF existente (Instância)
console.log('--- Teste de Validação ---');
const validador = new ValidaCPF('070.987.720-03');
if (validador.valida()) {
    console.log('CPF enviado é válido!');
} else {
    console.log('CPF enviado é inválido!');
}

// 2. Gerando um CPF novo do zero (Estático)
console.log('\n--- Fábrica de CPFs ---');
const cpfGerado = ValidaCPF.criaCpf();
console.log(`CPF Gerado automaticamente: ${cpfGerado}`);

// Prova real: Vamos validar o CPF que acabamos de gerar?
const testeGerado = new ValidaCPF(cpfGerado);
console.log(`O CPF gerado é válido? ${testeGerado.valida()}`);